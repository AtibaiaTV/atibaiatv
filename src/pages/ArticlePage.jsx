    ...
    <>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '2rem', display: 'grid', gridTemplateColumns: '1fr 300px', gap: '2rem' }}>
        <article>
          ...
          <h1 style={{ fontSize: '1.8rem', fontWeight: 700, color: 'var(--text)', lineHeight: 1.3, marginBottom: '1rem' }}>{news.title}</h1>
          ...
          {news.thumbnailUrl ? (
            <img src={news.thumbnailUrl} alt={news.title} style={{ width: '100%', height: 340, borderRadius: 10, marginBottom: '1.5rem', objectFit: 'cover' }} />
          ) : (
            <div style={{ width: '100%', height: 340, borderRadius: 10, marginBottom: '1.5rem', background: CARD_BG[news.color] || CARD_BG.blue, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          {related.length > 0 && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: '1rem' }}>
                <span style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--blue)' }}>Leia tambem</span>
